...
function MeetingItem(props){
    ...
    const itemForCalendarWeek = props.meetings.map((meeting) => {
        ...
        if (meeting.Date[2] === props.day && props.hour === (hourMeetingStart)) {
          ...
          const heigthMeeting = ((100 * (meeting.timeEnd[0] - meeting.timeStart[0]) + 100/60*(meeting.timeEnd[1]-meeting.timeStart[1]))+"%")
          const heigthMeetingBorder =((100 * (meeting.timeEnd[0] - meeting.timeStart[0]) + 100/60*(meeting.timeEnd[1]-meeting.timeStart[1]))/100 +"px") 
          const topMeeting = 100/60*(meeting.timeStart[1])+"%"

        console.log(heigthMeeting)
          return (
            <div
              style={{ background: `rgba(${meeting.typeMeeting.color})`, height: `calc(${heigthMeeting} + ${heigthMeetingBorder})`, top: topMeeting, position: "absolute" }}
