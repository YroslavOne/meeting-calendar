...
function SelectTime(props) {
  ...
  const minut = Number(value[3] + value[4])
  const prepand = hour > 12 ? " PM ":" AM "
  if (props.interactionWithTask) {
    return <div className="style-no-edit-timer">{twelveHour}<span className="react-time-picker__inputGroup__divider">:</span>{minut} {prepand}</div>;
  } else {
